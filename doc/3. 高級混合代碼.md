...
## 多行註解混合代碼v2
2026-04-28 多年後的今天 claude code 強大之下解出一個接近完美的解
原本第一版在某些邊界下還是會出問題，這一版已經降低更少了，缺點是代碼更黑魔法就是了

```ps1
<# ::BatchScript
@echo off & set "0=%~f0" & set "i=0"
:parse
if "%~1"=="" goto run
set /a i+=1 & call set "ARG_%%i%%=%%~1" & shift & goto parse
:run
set "ARGC=%i%"
powershell -nop "$t=0..[int]$env:ARGC|?{$_}|%%{$i=(gi \"env:ARG_$_\").Value; if($i -match '^-[\p{L}_]\w*(:.*)?$'){$i}else{\"'\"+$i.Replace(\"'\",\"''\")+\"'\"}}; iex ('& {'+[io.file]::ReadAllText($env:0)+'} '+(@($t) -join ' '))"
exit /b %errorlevel%
::PowerShellScript #>
[CmdletBinding()]
param(
...
```cmd
Demo.cmd -Path 'C:\app' "User Name" -F
```
