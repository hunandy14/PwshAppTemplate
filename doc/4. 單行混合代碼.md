...
## 單行混合v2
2026-04-28 沒想到AI居然想到還有 more +1 可以拿來跳過第一行，以及用 call 來延遲展開把 ExitCode 傳出去。  
只是有一個風險存在，這是複製的方式複製到暫存後執行，這行為可能會被多數的防毒檔下來。

```ps1
@echo off & set "_ORIG=%~f0" & more +1 "%~f0" > "%TEMP%\%~n0.ps1" & powershell -nop -File "%TEMP%\%~n0.ps1" %* & call set "_EC=%%errorlevel%%" & del "%TEMP%\%~n0.ps1" & call exit /b %%_EC%%
[CmdletBinding()]
param(
    [Parameter(Position=0)]
    [string]$Name,
