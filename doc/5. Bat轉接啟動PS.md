...
set "TSTAMP=%time: =0%"
set "LOG_FILE=%LOG_DIR%\%~n0_%date:~0,4%%date:~5,2%%date:~8,2%-%TSTAMP:~0,2%%TSTAMP:~3,2%%TSTAMP:~6,2%.log"
pwsh -exec bypass -noni -nop -c "& { try { cd '%~dp0'; & '%APP_NAME%' %*; exit $LASTEXITCODE } catch { $_; exit 1 } } *>&1 | Tee-Object -FilePath '%LOG_FILE%'"
exit /b %ERRORLEVEL%
```
